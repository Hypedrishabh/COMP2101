...
function error-message {
  ...
  echo "oops! we cant run the code anymore" >&2
...
function error-exit {
  ...
  echo "oops! we cant run the code anymore. here is why, $?"
...
function displayhelp {
  echo "
sysconfig.sh: This script shows information about the system. It includes options, -h | --help | --host | --domain | --ipconfig | --os | --cpu | --memory | --disk | --printer

There is no harm in trying all out but I think option names are pretty self explainatory.
"
}
...
function cleanup {
  rm /tmp/*.$$ && echo "tmp/*.$$ - temporary files for this process removed.."
  exit
}
