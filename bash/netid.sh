...
# Default Variables for script start
verbose="no"
rishabInterface=""
while [ $# -gt 0 ]; do
    case "$1" in
    -v | -verbose)
        ...
        ;;
    *)
        # Set passed interface name to scripts required interface
        [ "$rishabInterface" == "" ] && rishabInterface="$1"
        ;;
    esac
...
# interface="eno1"
if [ "$rishabInterface" == "" ]; then
    echo "No interface details are required"
else
    interface=$rishabInterface
